...
year = 365 * 24 * 60 * 60.0  # Normalizing time (1 year)
GG = (MM * G * year ** 2) / (RR ** 3)
...
final_time = 120  # final time = 120 years
points = 100 * final_time  # 100 points per year
t = np.linspace(initial_time, final_time, points)  # time array from initial_time to final_time with points points
t_step = t[2] - t[1]  # time step (uniform)
...
vvj = 13.06e3 * year / RR  # Magnitude of Jupiter's initial velocity
...
for i in range(0, points - 1):
    [r[i + 1, :], v[i + 1, :]] = RK4Solver(t[i], r[i, :], v[i, :], t_step, 'earth', rj[i, :], vj[i, :])
    [rj[i + 1, :], vj[i + 1, :]] = RK4Solver(t[i], rj[i, :], vj[i, :], t_step, 'jupiter', r[i, :], v[i, :])
